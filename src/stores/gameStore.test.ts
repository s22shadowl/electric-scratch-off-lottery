...
describe('selectCard', () => {
  ...
  it('重複選同一張牌不應重複加入 selectedCardIds', () => {
    ...
    useGameStore.getState().selectCard(cardId)
    useGameStore.getState().selectCard(cardId)
    expect(useGameStore.getState().selectedCardIds.filter(id => id === cardId)).toHaveLength(1)
  })
})
...
describe('updateCellProgress', () => {
  ...
  it(`進度達 REVEAL_THRESHOLD 時，cell.isRevealed 應為 true`, () => {
    ...
    const card = useGameStore.getState().cards[0]!
    const cellId = card.zone.cells[0]!.id
    useGameStore.getState().updateCellProgress(card.id, cellId, REVEAL_THRESHOLD)
    const updated = useGameStore.getState().cards.find(c => c.id === card.id)
    const cell = updated?.zone.cells.find(c => c.id === cellId)
  ...
  it('揭曉中獎格後，card.totalWinnings 應加上該獎金', () => {
    ...
    const cardWithWin = cards.find(c => c.zone.cells.some(cell => cell.prize.isWin))
    ...
    useGameStore.getState().updateCellProgress(cardWithWin!.id, winCell.id, REVEAL_THRESHOLD)
    const updated = useGameStore.getState().cards.find(c => c.id === cardWithWin!.id)
  ...
  it('所有 cell 揭曉後，card.status 應變為 completed', () => {
    ...
    card.zone.cells.forEach(cell => {
      useGameStore.getState().updateCellProgress(card.id, cell.id, REVEAL_THRESHOLD)
    })
    const updated = useGameStore.getState().cards.find(c => c.id === card.id)
