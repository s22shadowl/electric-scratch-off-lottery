import type { Prize, ScratchCard, ScratchZone, ScratchCell, GameConfig } from '@/types'
...
// 建立單一刮除格
function buildCell(cellIndex: number, cardId: string, prizes: Prize[]): ScratchCell {
  return {
...
// 建立單一刮除區
function buildZone(cardId: string, cellCount: number, prizes: Prize[]): ScratchZone {
  return {
    id: `${cardId}-zone`,
    shapeVariant: 'single',
    cells: Array.from({ length: cellCount }, (_, i) => buildCell(i, cardId, prizes)),
  }
}

// 建立一張刮刮樂卡（在建卡時依機率分配獎項）
export function buildCard(config: GameConfig, cardId: string): ScratchCard {
