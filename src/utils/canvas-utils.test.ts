...
import { drawErase, drawSilverMask, calculateRevealedRatio } from './canvas-utils'
...
function makeMockCtx(alphaValues?: number[]) {
  ...
  // 若有指定，覆蓋 alpha 值
  alphaValues?.forEach((a, idx) => { data[idx * 4 + 3] = a })
  ...
  return {
    ...
    getImageData: vi.fn(() => ({ data })) as unknown as CanvasRenderingContext2D['getImageData'],
  }
}
...
describe('calculateRevealedRatio', () => {
  it('全不透明時應回傳 0', () => {
    ...
    const ratio = calculateRevealedRatio(ctx as unknown as CanvasRenderingContext2D, 4, 4)
    expect(ratio).toBe(0)
  })
  ...
  it('空 canvas（width=0）應回傳 0', () => {
    const ctx = makeMockCtx()
    const ratio = calculateRevealedRatio(ctx as unknown as CanvasRenderingContext2D, 0, 0)
    expect(ratio).toBe(0)
  })
})
