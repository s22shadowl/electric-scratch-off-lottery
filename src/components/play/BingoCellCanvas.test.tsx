...
describe("BingoCellCanvas", () => {
  it("未揭曉時應渲染 canvas 遮罩", () => {
    const cell = makeCell();
    render(
      <BingoCellCanvas cell={cell} cardId="card-1" isMatched={false} />,
    );
  ...
  it("應顯示 bingoNumber", () => {
    const cell = makeCell({ isRevealed: true, scratchProgress: 1, bingoNumber: 42 });
    render(
      <BingoCellCanvas cell={cell} cardId="card-1" isMatched={false} />,
    );
  ...
  it("已揭曉且 isMatched=true 時應有 data-matched 屬性", () => {
    const cell = makeCell({ isRevealed: true, scratchProgress: 1 });
    render(
      <BingoCellCanvas cell={cell} cardId="card-1" isMatched={true} />,
    );
    expect(
      screen.getByTestId(`bingo-cell-${cell.id}`),
  ...
  it("已揭曉且 isMatched=false 時 data-matched 應為 false", () => {
    ...
    expect(
      screen.getByTestId(`bingo-cell-${cell.id}`),
    ).toHaveAttribute("data-matched", "false");
  });
