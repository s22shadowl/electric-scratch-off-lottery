...
- **卡堆刮刮樂**：多張刮刮時左右兩側露出已刮/未刮卡片邊緣，仿手疊散牌實體感；刮完後右側下一張發光可點擊
...
| 標準     | 95%              | 一般聚會                                 |
| 保守     | 80%              | 競爭感更強                               |
| 真實     | 63%（仿台彩）    | 7 層獎項含 $300,000 頭獎，貼近真實體驗   |
| 派對     | 95%（計畫中）    | 7 層獎項含 $50,000 頭獎，頭獎可中（尚未實作 UI 選項） |
...
- [x] SymbolLayout 視覺精緻化（背景色、格子形狀、格內內容、標題、裝飾、密度）— Phase 1-3 完成
- [ ] SymbolLayout 素材升級（emoji 裝飾 → 高解析插畫，等素材到位）
- [ ] Triple ThemeLayout（同 SymbolLayout 等級：absolute 定位、雙尺寸、手工座標）
- [ ] Compare ThemeLayout
- [ ] Bingo ThemeLayout
- [x] PlayPage 卡堆模式（scratching phase：隱藏 header 放大卡片、左右卡堆顯示已刮/未刮、手散亂偏移、完成後右堆發光可點擊切換）
- [ ] 多款式選擇（節慶、生肖等主題）
